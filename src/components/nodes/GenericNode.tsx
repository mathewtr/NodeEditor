...
export function GenericNode({ id, data, type }: NodeProps) {
  ...
  return (
    <div className="node">
      ...
      {/* Connector node: app parameter display */}
      {data.appParameter && (
        <div className="node-params">
          <div className="connector-param">{data.appParameter}</div>
        </div>
      )}

      {/* Parameter fields (with optional connection handles) */}
      {paramEntries.length > 0 && !data.appParameter && (
        <div className="node-params">
          {paramEntries.map((param) => (
            <NodeField
              key={param.name}
              label={param.name}
              value={data[param.name] as number | number[]}
              nodeId={id}
